...
void  TMForm::SaveINIFile(){
	char filename[] = "SET.ini";
		...
		if(bGL_PD_VPID)
			...
			fp << "false" << endl;
		if(strGL_PD_VPID == "" || strGL_PD_VPID == NULL )
			fp << "NULL" << endl;
		else
			fp << strGL_PD_VPID.c_str() << endl;
...
void  TMForm::Read_INI_File(){
	...
	if(FileExists("SET.ini"))
	{
		...
		bGL_PD_VPID          = Findfilemsg("SET.ini", "<GL-PDVersion>", 2) =="true" ? true:false;
		if(bGL_PD_VPID)
			strGL_PD_VPID = Findfilemsg("SET.ini", "<GL-PDVersion>", 3);
		...
		for(int i=1;i<=15;i++)
		{
			...
			edtType->Text = deviceinfo[i-1].SubString(deviceinfo[i-1].Pos("[")+1
							,deviceinfo[i-1].Pos("]")-deviceinfo[i-1].Pos("[")-1);
...
void TMForm::CheckDevVPID(int flag)
{
	...
	for(int i = 1; i <= DeviceI;i++)
	{
		...
		{   //allpass 預防有顏色顯示錯誤的
			...
			bAllClear = false;
			if(bGL_PD_VPID && deviceVPID == strGL_PD_VPID)
				bGL_PD_VPID_PASS = true;
		}
	...
	if(plGLPDView->Height)
	{
		if(bAllPass || bGL_PD_VPID_PASS)
		{
		...
		else if(bAllClear && plGLPDResult->Color != clCream)
		{
			plGLPDResult->Caption = "WAIT";
			plGLPDResult->Color = clCream;
        }
