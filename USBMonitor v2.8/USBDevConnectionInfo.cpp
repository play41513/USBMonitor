...
DWORD USBDevConnectionInfo::GetUSBType(AnsiString USBInfo)
{
	TListUSBDEV_CONN_INFO->Clear();
	AnsiString VID = USBInfo.SubString(USBInfo.Pos("vid")+4,4).UpperCase();
	AnsiString PID = USBInfo.SubString(USBInfo.Pos("pid")+4,4).UpperCase();
	AnsiString Path;
	if(USBInfo.Pos("{"))
	...
	for (int i=0; i<10; i++)
	{
		...
		{
			EnumerateHostController(HostControllerHandle,Path);
		}
	...
	{
		if (TListUSBDEV_CONN_INFO->Strings[j].Pos(Path))//確認裝置路徑碼
		{
			j+=3;
			a = TListUSBDEV_CONN_INFO->Strings[j];
			if (TListUSBDEV_CONN_INFO->Strings[j].Pos("VID :"+VID))//確認裝置VID
			{
				j++;
				a = TListUSBDEV_CONN_INFO->Strings[j];
				if (TListUSBDEV_CONN_INFO->Strings[j].Pos("PID :"+PID))//確認裝置PID
				{
					while(j+1!=TListUSBDEV_CONN_INFO->Count)
					{
						j++;
						a = TListUSBDEV_CONN_INFO->Strings[j];
						if(TListUSBDEV_CONN_INFO->Strings[j].Pos("Bus Speed"))
						{
							if(TListUSBDEV_CONN_INFO->Strings[j].Pos("SuperSpeedPlus"))
							{
								return USBTYPE_SUPER_SPEED_PLUS;
							}
							else if(TListUSBDEV_CONN_INFO->Strings[j].Pos("SuperSpeed"))
							{
								return USBTYPE_SUPER_SPEED;
							}
							else if(TListUSBDEV_CONN_INFO->Strings[j].Pos("Hi-Speed"))
							{
								return USBTYPE_HI_SPEED;
							}
							else if(TListUSBDEV_CONN_INFO->Strings[j].Pos("FullSpeed"))
							{
								return USBTYPE_HI_SPEED;
							}
							else if(TListUSBDEV_CONN_INFO->Strings[j].Pos("LowSpeed"))
							{
								return USBTYPE_HI_SPEED;
							}
							else
							{
								return USBTYPE_LOW_SPEED_PLUS;
							}
						}
...
DWORD USBDevConnectionInfo::GetUSBType(AnsiString VID,AnsiString PID,AnsiString Path)
{
	TListUSBDEV_CONN_INFO->Clear();
	HANDLE HostControllerHandle;
	char HostControllerName[] = "\\\\.\\HCD0";
...
DWORD USBDevConnectionInfo::EnumerateHostController(HANDLE HostControllerHandle,AnsiString Path)
{
  DWORD BytesReturned;
  bool Success;
	...
	if(Success)
	{
	  ...
	  if(RootHubHandle != INVALID_HANDLE_VALUE)
	  {
        ...
        else
		{
		  ...
		  GetPortData(RootHubHandle, info->HubInfo->u.HubInformation.HubDescriptor.bNumberOfPorts, 0,Path);
        }
      }
...
//---------------------------------------------------------------------------
void USBDevConnectionInfo::GetPortData(HANDLE HubHandle, UCHAR PortCount, int HubDepth,AnsiString Path)
{
  DWORD BytesReturned;
  bool Success;
  int i;
  ...
  for (PortIndex = 1; PortIndex < (ULONG)PortCount + 1; PortIndex++)
  {
	...
	if (PortStatus == DeviceConnected)
	{
	  ...
	  if (!ConnectionInformation.DeviceIsHub)
	  {
		...
		TListUSBDEV_CONN_INFO->Add("Bus Speed : "+GetSpeed(HubHandle,PortIndex));
		//USB裝置找到即跳出迴圈
		break;
		//2020/2/27 取消舊版判斷USB2 USB3
        ...
        if (Success)
		{
          ...
          if(ConnectedHubHandle != INVALID_HANDLE_VALUE)
		  {
			...
			if (Success)
			{
			  ...
			  // Get the ports and take the data
				TListUSBDEV_CONN_INFO->Add("Bus Speed : "+GetSpeed(HubHandle,PortIndex));
				GetPortData(ConnectedHubHandle, info->HubInfo->u.HubInformation.HubDescriptor.bNumberOfPorts, HubDepth+1,Path);
			  }
			  else
			  {
				  TListUSBDEV_CONN_INFO->Add("Bus Speed : LowSpeed\n");
				  GetPortData(ConnectedHubHandle, info->HubInfo->u.HubInformation.HubDescriptor.bNumberOfPorts, HubDepth+1,Path);
			  }
			}
			else
...
{
	if(TListUSBDEV_CONN_INFO->Count)
		return TListUSBDEV_CONN_INFO->Text;
	return "";
}
