	...
	: TForm(Owner)
{
	...
	if(IsWow64)	DEVCON = "devcon64";
	else DEVCON = "devcon32";
...
{
	AnsiString Resultdata = DosCommand(DEVCON+" hwids \"\*usb\\\*\"");
	bool Hub = false;
	...
	while (pch != NULL) {
		...
		{
			if(((AnsiString)NList->Text).Pos("USB20_HUB")||((AnsiString)NList->Text).Pos("USB30_HUB"))
			{//判斷為hub
				bool bDevIsHub3 = ((AnsiString)NList->Text).Pos("USB30_HUB") ? true:false;
				for(int x=0;x<NList->Count;x++)
				{
					if(wcsstr(NList->Strings[x].c_str(), L"REV_"))
					{
						if(!bSaveLocalDevInfo){
							TListInitUSB_Dev->Add((AnsiString)NList->Strings[x]);
						}
						else if(!lan_find_inf(((AnsiString)TListInitUSB_Dev->Text).c_str(),((AnsiString)NList->Strings[x]).c_str()))
						{
							Result=(AnsiString)NList->Strings[x];
							int Infolength = Result.Length();
							if(Infolength > 38)
							{
									if(bDevIsHub3) TListHUB_Dev->Add("[HUB_3]"+Result.SubString(8,38));
									else TListHUB_Dev->Add("[HUB_2]"+Result.SubString(8,38));
							}
							else
							{
									if(bDevIsHub3) TListHUB_Dev->Add("[HUB_3]"+Result.SubString(8,38));
				...
				for(int x=0;x<NList->Count;x++)
				{
					if(wcsstr(NList->Strings[x].c_str(), L"REV_"))
					{
						...
						else if(!lan_find_inf(((AnsiString)TListInitUSB_Dev->Text).c_str(),((AnsiString)NList->Strings[x]).c_str()))
						{
							...
							{
									TListUSB_Dev->Add(Result.SubString(8,Infolength));
							}
						}
						break;
					}
...
void TMForm::CheckDiskName()
{
	if(!bPLUGOUT)
	{
		...
		if(moDisk->Lines->Count==0)
		{
			for(int j=0;j<16;j++)
			{
				if(usb_rw_info[j].zDeviceTable->Color!=clCream)
				{
