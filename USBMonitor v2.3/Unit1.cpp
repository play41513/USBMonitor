...
void  TMForm::Read_INI_File(){
	...
	if(FileExists("SET.ini"))
	{
		...
		if(DeviceI == 0)
			...
			plLoadDUTData ->Height = Panel9->Height+DeviceI*35;
	...
	else
	{
		...
		dwINITimeOut = 5000;
		DeviceI =  1;
		plLoadDUTData ->Height = Panel9->Height + 35;
	}
...
void __fastcall TMForm::btnLoadDUTClick(TObject *Sender)
{
	if(MessageBox(0,"請確認程式畫面上DUT HUB / USB資訊是否完整\n按  \"是\"  載入DUT資訊 \n","警告",MB_YESNO) == IDYES)
	{
		...
		else plLoadDUTData ->Height = Panel9->Height + (setVPIDpl-1)*35;
		MForm->Height = 390+plLoadDUTData ->Height;
		saveinfo();
	}
}
	...
	for(int x=0;x<ts->Count;x++)
	{
		...
		for(int y=3;y<26;y++)
		{
			...
			if(bBuf != NULL)
			{
				...
				&&DiskVolumelabel == AnsiString(disbuf).SubString(13,1))
				{
					...
					delete ts;
					delete reg;
					return USBCONInfo->GetUSBType(VID,PID,DevPath);
				}
			}
			//
			free(bBuf);
