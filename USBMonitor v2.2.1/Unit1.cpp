...
void  TMForm::saveset(){
	char filename[] = "SET.ini";
		...
		if(deviceinfo1 != ""){
			fp << "<DeviceInfo1>" << AnsiString(edtNum1->Text).c_str() <<endl;
			fp << deviceinfo1.c_str() << endl;
		}
		if(deviceinfo2 != ""){
			fp << "<DeviceInfo2>" <<  AnsiString(edtNum2->Text).c_str() <<endl;
			fp << deviceinfo2.c_str() << endl;
		}
		if(deviceinfo3 != ""){
			fp << "<DeviceInfo3>" <<  AnsiString(edtNum3->Text).c_str() <<endl;
			fp << deviceinfo3.c_str() << endl;
		}
		if(deviceinfo4 != ""){
			fp << "<DeviceInfo4>" <<  AnsiString(edtNum4->Text).c_str() <<endl;
			fp << deviceinfo4.c_str() << endl;
		}
		if(deviceinfo5 != ""){
			fp << "<DeviceInfo5>" <<  AnsiString(edtNum5->Text).c_str() <<endl;
			fp << deviceinfo5.c_str() << endl;
		}
		if(deviceinfo6 != ""){
			fp << "<DeviceInfo6>" <<  AnsiString(edtNum6->Text).c_str() <<endl;
